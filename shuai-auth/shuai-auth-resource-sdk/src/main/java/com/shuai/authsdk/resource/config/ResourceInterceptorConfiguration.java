package com.shuai.authsdk.resource.config;


import com.shuai.authsdk.resource.interceptors.LoginAuthInterceptor;
import com.shuai.authsdk.resource.interceptors.UserInfoInterceptor;
...
public class ResourceInterceptorConfiguration implements WebMvcConfigurer {
    ...
    public void addInterceptors(InterceptorRegistry registry) {
        ...
        // 2.1.添加拦截器路径
        if(!CollectionUtils.isEmpty(authProperties.getIncludeLoginPaths())){
            registration.addPathPatterns(authProperties.getIncludeLoginPaths());
        }
        // 2.2.添加排除路径
        if(!CollectionUtils.isEmpty(authProperties.getExcludeLoginPaths())){
